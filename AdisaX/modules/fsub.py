...
from AdisaX import BOT_ID
from AdisaX import DRAGONS as DEVS
from AdisaX import OWNER_ID
from AdisaX import telethn as Adisa
from AdisaX.events import Dilinline
from AdisaX.events import register as AdisaXBot
from AdisaX.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await Adisa(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await Adisa(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@AdisaXBot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
...
*ғᴏʀᴄᴇ ꜱᴜʙꜱᴄʀɪʙᴇ:*
   ...
   •➥ *ᴅɪʟʙᴏᴛs ᴄᴀɴ ᴍᴜᴛᴇ ᴍᴇᴍʙᴇʀꜱ ᴡʜᴏ ᴀʀᴇ ɴᴏᴛ ꜱᴜʙꜱᴄʀɪʙᴇᴅ ʏᴏᴜʀ ᴄʜᴀɴɴᴇʟ ᴜɴᴛɪʟ ᴛʜᴇʏ ꜱᴜʙꜱᴄʀɪʙᴇ*
   ...
   •➥ *ꜱᴇᴛᴜᴘ*
   •➥ [ᴀᴅᴅ ᴍᴇ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/adisamanagerbot?startgroup=new)
   •➥ [ᴀᴅᴅ ᴍᴇ ɪɴ your ᴄʜᴀɴɴᴇʟ ᴀꜱ ᴀᴅᴍɪɴ](https://t.me/adisamanagerbot?startgroup=new)
