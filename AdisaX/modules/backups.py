...
import AdisaX.modules.sql.blacklist_sql as blacklistsql

import AdisaX.modules.sql.locks_sql as locksql
import AdisaX.modules.sql.notes_sql as sql

import AdisaX.modules.sql.rules_sql as rulessql
from AdisaX import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from AdisaX.__main__ import DATA_IMPORT
from AdisaX.modules.connection import connected
from AdisaX.modules.helper_funcs.alternate import typing_action
from AdisaX.modules.helper_funcs.chat_status import user_admin
from AdisaX.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("AdisaX{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("AdisaX{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `AdisaX-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("AdisaX{}.backup".format(chat_id))  # Cleaning file
