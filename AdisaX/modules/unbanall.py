...
from AdisaX.events import register
...
async def _(event):
    ...
    is_admin = False
    try:
        dilrobot = await telethn(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        is_admin = False
    else:
        if isinstance(
            dilrobot.participant,
            (
                ChannelParticipantAdmin,
                ChannelParticipantCreator,
            ),
...
async def _(event):
    ...
    is_admin = False
    try:
        AdisaX = await telethn(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        is_admin = False
    else:
        if isinstance(
